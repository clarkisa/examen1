...
// Giro 180° (tuning)
// Giro 180° (tuning)
#define TURN_TIME_MS     1200u   // antes 650u → ~x2 para ~180° esto cambio para tiempo de giro 
...
void GPIOAIntHandler(void){
    ...
    if(pin){
        // flanco subida → inicio de pulso
        us_echo_rise = now;
    }else{                     // flanco de BAJADA
        uint32_t ticks = now - us_echo_rise;   // wrap ok con unsigned
        uint32_t width_us = ticks_to_us(ticks);
        us_last_width = width_us;
        // filtros rápidos contra ruido
        if(width_us < 100u || width_us > 25000u){   // ~1..25000us ≈ 0.5..400cm
            us_last_cm = -1;
        }else{
...
// Dispara giro solo si no estamos en "ignore" (cooldown)
if (us_ignore_ms == 0 && us_last_cm > 0 && us_last_cm <= STOP_CM) {
    // Freno inmediato + PWM=0
    A_brake(); B_brake();
    MAP_PWMPulseWidthSet(PWM0_BASE, PWM_OUT_1, 0);
    MAP_PWMPulseWidthSet(PWM0_BASE, PWM_OUT_2, 0);
    g_motion = M_STOP;

    // Señaliza al lazo principal que debe girar 180°
    g_need_turn = true;
    us_hard_stop = true;  // (opcional: para log)
}
...
void UART0IntHandler(void){
    ...
    break;
          ...
          //  case 'w': g_motion = M_FWD;   apply_motion(); break;   ESTO PARA QUE EL TECLADO FUNCIONE BIEN
    ...
    break;
            ...
            //case ' ': g_motion = M_STOP;  apply_motion(); break;   ESTO PARA QUE EL TECLAOD FUNCIONE BIEN
...
// Ejecutar el giro 180° cuando la ISR lo pida
if (g_need_turn) {
    g_need_turn = false;

    // Ignora lecturas de ultrasonido mientras giramos (y un poquito más)
    us_ignore_ms = TURN_TIME_MS + TURN_COOLDOWN_MS;

    // Freno corto antes de girar (mejora la precisión)
    A_brake(); B_brake();
    delay_us(80 * 1000u); // ~80 ms

    // Giro en el sitio: Motor A adelante, Motor B atrás (o al revés si quieres girar al otro lado)
    A_forward();
    B_reverse();
...
set_pwm_B(TURN_B_DUTY_PCT);
    ...
    // Mantén el giro el tiempo calibrado
    delay_us(TURN_TIME_MS * 1000u);
...
// Reanuda sólo si el auto-run está activo
if (g_auto_run) {
    g_motion = M_FWD;
    apply_motion();
} else {
...
// Auto-run: si está habilitado y no estamos girando ni ignorando eco, avanza
if (g_auto_run && us_ignore_ms == 0 && !g_need_turn) {
    // solo arranca si no estamos ya en FWD, el frente está libre y el auto-run sigue activo
