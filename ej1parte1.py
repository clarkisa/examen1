...
def autodetect_port():
    ...
    if lin: return lin[0]
    raise RuntimeError("No serial device found (/tty.usbmodem*, /ttyACM*, /ttyUSB*)")
...
Controls (hold to move):
  ...
  SPACE/0  → stop
  b        → buzzer (2s)
  k / l    → duty down / up  (5,25,50,75,100)
Exit:
  q / ESC / Ctrl+C
"""
...
def main():
    ...
    try:
        ...
        print(f"[ERROR] Cannot open {port}: {e}"); sys.exit(1)
    ...
    try:
        with RawKB() as kb, ser:
            while True:
                ...
                # autorepeat WASD mientras mantienes
                ...
                # muestra telemetría (STATE/DUTY/DIST)
                try:
